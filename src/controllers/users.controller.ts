...
export const updateUserHandler = async (
  request: FastifyRequest<{
    Params: { id: string };
    Body: { firstName?: string; lastName?: string; email?: string; role?: string; department?: string; isActive?: boolean };
  }>,
  reply: FastifyReply
) => {
  const currentUser = request.user;
  ...
  try {
    ...
    return reply.status(400).send({ success: false, message: err.message || 'Failed to update user' });
  }
};
...
export const deleteUserHandler = async (
  ...
  try {
    ...
    await auditService.log({
      action: SecurityEvents.USER_DELETED,
      user: currentUser,
      resourceType: 'User',
      resourceId: id,
      request,
    ...
    return reply.status(400).send({ success: false, message: err.message || 'Failed to delete user' });
  }
};
